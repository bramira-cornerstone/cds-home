...
export type EditionCardMiniProps = {
  id: number;
  name?: string | null;
  thumb?: string | null;
  tier?: string | null;
...
function EditionCardMini(props: EditionCardMiniProps) {
  const {
    id,
    name,
    thumb,
  ...
  if (!isDataReady) {
    return (
      <div
        className="h-full w-full max-sm:h-[98%] max-sm:w-[95%] card-shadow-responsive flex flex-col"
        style={{
          paddingBottom: "2px",
          margin: "0 auto",
          width: "97%",
  ...
  return (
    ...
    >
      <div className="relative flex-1 w-full rounded-[1px] border border-slate-200 bg-white nightmode-exempt overflow-hidden flex flex-col m-auto card-shadow">
        ...
        <div
          className="text-[10px] text-slate-700 truncate flex-shrink-0"
        >
          {minted != null
            ? `${String(minted)} to ever exist`
            : `edition_id: ${id}`}
        </div>
        {gameDate ? (
        ...
        {setName ? (
          <div
            className="mt-[2px] mb-[6px] text-[10px] max-sm:text-[9px] font-normal text-slate-700 text-center truncate flex-shrink-0"
            title={setName}
          >
            {setName}
          </div>
...
export default memo(EditionCardMini);
